...
class BoostedTrees:
    ...
    def train(self):
        ...
        # List to store base classifiers and weights for each base classifier
        ...
        for t in range(L):
            ...
            # Compute the error and alpha on the predictions.
            train_predictions = np.array(self.predict(X_train))
            epsilon = D.dot(train_predictions != y_train)
            alpha = np.log((1 - epsilon) / epsilon) / 2

            # Convert class label 0 to -1 so weights update properly below.
            _y_train = np.array([-1 if y == 0 else 1 for y in y_train])
            _train_predictions = np.array([-1 if y == 0 else 1 for y in train_predictions])

            # Update weights for next classifier and normalize.
            D = D * np.exp(-alpha * _y_train * _train_predictions)
            D = D / D.sum()

            # Save alpha for predictions.
